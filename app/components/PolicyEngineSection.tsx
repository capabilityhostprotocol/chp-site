        ...
        <div>
          ...
          <div className="space-y-3">
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
              <div className="flex items-center gap-1.5 px-4 py-2.5 border-b border-zinc-800">
                ...
                <span className="w-2 h-2 rounded-full bg-zinc-700" />
                <span className="w-2 h-2 rounded-full bg-zinc-700" />
                <span className="ml-2 font-mono text-xs text-zinc-400">.chp/policy.json</span>
              </div>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
              <div className="flex items-center gap-1.5 px-4 py-2.5 border-b border-zinc-800">
                ...
                <span className="w-2 h-2 rounded-full bg-zinc-700" />
                <span className="w-2 h-2 rounded-full bg-zinc-700" />
                <span className="ml-2 font-mono text-xs text-zinc-400">result</span>
              </div>
              <pre className="p-4 font-mono text-xs text-zinc-300 overflow-x-auto leading-relaxed">
          ...
          <div className="flex flex-wrap gap-1.5 mt-3">
            {['chp policy lint', 'audit_only: true', 'block_patterns', 'max_risk_tier'].map((tag) => (
              <span key={tag} className="font-mono text-xs text-zinc-400 bg-zinc-900 border border-zinc-800 rounded px-1.5 py-0.5">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {MATURITY_LEVELS.map(({ level, label, desc }) => (
              ...
              >
                <span className="font-mono text-xs text-zinc-400 min-w-[1.75rem] mt-0.5">{level}</span>
                <div className="flex-1 min-w-0">
                  <span className="text-xs font-semibold text-zinc-300">{label}</span>
                  <span className="text-xs text-zinc-400 ml-2">{desc}</span>
                </div>
              </div>
            ))}
          </div>
